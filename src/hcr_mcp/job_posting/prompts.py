...
구조화 규칙:
- 모집분야가 하나뿐이어도 그 하나를 반드시 jobs[0]에 원소로 넣어 리스트 형태로 반환하세요
  — 직무가 1개라고 배열이 아닌 다른 형태로 반환하지 마세요.
- [공고 원문]이 비어 있거나 채용공고로 파싱할 내용이 전혀 없으면(스크래핑/추출 실패 등)
  jobs는 빈 배열로 반환하세요 — 없는 직무명을 지어내지 마세요.
