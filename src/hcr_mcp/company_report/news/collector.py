...
_DATE_REL_RE = re.compile(r"(\d+)\s*(분|시간|일|주)\s*전")
_DATE_REL_UNIT_DAYS = {"분": 0, "시간": 0, "일": 1, "주": 7}  # "일"만 처리하던 걸 "주"(week) 누락 발견 후 확장(실측: "1주 전"이 빈 문자열로 새던 걸 확인)
...
def _parse_date_text(text: str) -> str:
    ...
    if m:
        n, unit = m.groups()
        delta = timedelta(days=int(n) * _DATE_REL_UNIT_DAYS[unit])
...
class _IssueSummaryItem(BaseModel):
    ...
    detail_summary: str = Field(
        description=(
            "actor를 주어로 3~5문장 상세 요약 — 구직자가 자기소개서·면접에서 이 이슈를 구체적으로 "
            "언급할 수 있도록 본문에 실제로 있는 날짜·금액·인물명·수치 등을 포함. gist를 풀어 쓴 "
            "것이며, 본문에 없는 해석·추측·결론은 절대 추가하지 않는다(gist보다 길다고 새로운 "
            "사실을 지어내도 된다는 뜻이 아님)"
        )
    )
...
2. 그다음 actor를 주어로 핵심 내용을 1~2문장으로 요약하세요(제목을 반복하지 말고 본문에만
   있는 구체적인 내용을 담으세요) — 이게 gist입니다.
3. gist를 3~5문장으로 풀어서 detail_summary를 쓰세요 — 본문에 실제로 있는 날짜·금액·인물명·
   수치 등 구체적인 사실을 담되, **본문에 없는 내용(해석, 추측, 전망, 결론)은 절대 추가하지
   마세요.** 문장이 길어진다고 새로운 사실이 생기는 게 아닙니다 — 본문에 없으면 짧게 끝내세요.
4. 위 분류체계에서 가장 적합한 event_id 하나로 분류하세요. 애매하면 general_other.general_pr을 쓰세요.
...
async def _summarize_and_classify_batch(issues: list[dict]) -> list[dict]:
    ...
    for i in range(len(issues)):
        item = by_index.get(i)
        if item:
            results.append({"gist": item.gist, "detail_summary": item.detail_summary, "event_id": item.event_id})
        else:
            results.append({"gist": "", "detail_summary": "", "event_id": event_taxonomy._FALLBACK_EVENT_ID})  # 이 항목만 개별 폴백
    return results
...
def _merge_timelines(finalized: list[dict], reconstructions: list[dict]) -> list[dict]:
    """supersedes로 이어진 이슈들을 체인으로 묶어 하나로 병합한다. 병합된 이슈의 gist/detail_summary는
    LLM이 새로 쓰는 게 아니라, 이미 나온 문장들을 시간순으로 화살표로 이어붙인 것뿐이다(파이썬
    문자열 조립) — 여러 사건을 하나의 새 문장으로 재서술시키면 그 단계에서 환각이 생기기 쉽다는
    연구 결과가 있어(타임라인 요약에서 "날짜별 사건 통합" 단계가 주요 환각 원인 중 하나), 새
    문장을 쓰게 하지 않고 기존 문장을 그대로 잇는 방식을 택했다. 기사(출처)도 전부 보존한다."""
    n = len(finalized)
    parent = list(range(n))
    ...
    for members in chains.values():
        ...
        if len(members) > 1:
            ...
            detail_summary = " → ".join(f"[{finalized[i]['occurred_month']}] {finalized[i]['detail_summary']}" for i in members)
        else:
            gist = latest["gist"]
            detail_summary = latest["detail_summary"]
...
async def _finalize_issues(issues: list[dict]) -> tuple[list[dict], list[dict]]:
    """gist·상세요약·분류·중요도 재평가·시간대 연결을 붙여 토픽별로 묶은 최종본과, 오류 대비용
    원문(본문 포함) 보존본을 함께 반환한다. 최종본의 이슈는 title/url/press/date(출처)+gist+
    detail_summary+event_id+importance만 남기고, 원문 보존본은 저장 전용(프롬프트에는 안 넣음)
    — 요약이 잘못됐거나 재처리가 필요할 때 재스크래핑 없이 다시 쓸 수 있게 한다."""
    if not issues:
    ...
    finalized = [
        {
            ...
            "gist": s["gist"],
            "detail_summary": s["detail_summary"],
            "event_id": s["event_id"],
